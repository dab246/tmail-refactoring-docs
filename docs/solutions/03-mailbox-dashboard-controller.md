...
**Vấn đề:** God Object 3,508 lines, 43 deps, 42 Rx fields, 15 bounded contexts.
...
class EmailListStateService extends GetxService {
  ...
  // Mailbox maps — move ra khỏi MailboxDashBoardController
...
class EmailActionController extends GetxController {
  ...
  // Fix Mixed Abstraction: quyết định dùng interactor nào nằm đây
  void moveEmail({
  ...
  // Fix Bumpy Road: handleSuccess chỉ xử lý email action success types
...
**Trước:**
```dart
// 1 method xử lý 31 types từ 15 bounded contexts
...
**Sau:** Mỗi sub-controller tự override `handleSuccessViewState()` và chỉ handle các types thuộc về nó.
...
void handleSuccessViewState(Success success) {
  switch (success) {
    ...
    default: super.handleSuccessViewState(success); // pass up nếu không handle
  }
}
...
**Sau — nhóm theo bounded context trong services:**
```dart
// DragDropStateService
...
// SearchStateService (hoặc SearchEmailController)
...
## MailboxDashBoardController sau refactor (~200 lines)
...
class MailboxDashboardBindings extends Bindings {
  ...
  void dependencies() {
    ...
    // Orchestrator — nhận services qua constructor
